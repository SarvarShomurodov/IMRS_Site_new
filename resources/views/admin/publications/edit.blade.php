  ...
  <div class="content">
              ...
              <div class="row align-items-center">
                  <div class="col-sm-6">
                      ...
                      <ol class="breadcrumb">
                          ...
                          <li class="breadcrumb-item active">Изменить новость</li>
                      </ol>
                  </div>
                  <div class="col-sm-6">
                      <div class="float-right d-none d-md-block">
                          <div class="dropdown">
                              <a class="btn btn-primary dropdown-toggle arrow-none waves-effect waves-light text-white" href="/admin/news/create">
      ...
      <div class="row">
          <div class="col-12">
              <div class="card">
                  <div class="card-body">
                      <h4 class="mt-0 header-title">Новости</h4>
                      <br><br><br>

                      <form class="needs-validation" action="/admin/publications/edit/{{$item->id}}" method="post" enctype='multipart/form-data'>
                        @csrf
                                ...
                                @foreach($categories as $archive)
                                  <option value="{{$archive->id}}"
                                    @foreach($item->categories as $category)
                                      @if($category->id==$archive->id)
                                        selected
                                      @endif
                                    @endforeach
                                    @if($archive->child()->exists()) disabled @endif
                                    >{{$archive->title_ru}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Фото (uz)</label>
                            <img src="/images/publications/uz/{{$item->image_uz}}" alt="" width="300">
                        ...
                        <div class="form-group">
                            <label>Фото (ru)</label>
                            <img src="/images/publications/ru/{{$item->image_ru}}" alt="" width="300">
                        ...
                        <div class="form-group">
                            <label>Фото (en)</label>
                            <img src="/images/publications/en/{{$item->image_en}}" alt="" width="300">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description_uz" id="description_uz">{!!$item->description_uz!!}</textarea>
                        </div>
                        <div class="form-group">
                            <label>Детали (ru)</label>
                            <textarea name="description_ru" id="description_ru">{!!$item->description_ru!!}</textarea>
                        </div>
                        <div class="form-group">
                            <label>Детали (en)</label>
                            <textarea name="description_en" id="description_en">{!!$item->description_en!!}</textarea>
                        </div>

                        <div class="form-group">
                            <label>pdf (uz)</label>
                            <object data="/files/publications/uz/{{$item->pdf_uz}}" type="application/pdf" width="580" height="300">
                              <a href="/files/news/uz/{{$item->pdf_uz}}">{{$item->pdf_uz}}</a>
                            </object>
                        ...
                        <div class="form-group">
                            <label>pdf (ru)</label>
                            <object data="/files/publications/ru/{{$item->pdf_ru}}" type="application/pdf" width="580" height="300">
                              <a href="/files/news/ru/{{$item->pdf_ru}}">{{$item->pdf_ru}}</a>
                            </object>
                        ...
                        <div class="form-group">
                            <label>pdf (en)</label>
                            <object data="/files/publications/en/{{$item->pdf_en}}" type="application/pdf" width="580" height="300">
                              <a href="/files/news/en/{{$item->pdf_en}}">{{$item->pdf_ru}}</a>
                            </object>
                        ...
                        @foreach($item->files as $file)
                          @php $name = explode('.', $file->file); @endphp
                          <div class="" style="display: flex;  align-items:center">
                            @if($name[1]=='pdf')
                            <object data="/files/files/{{$file->file}}" type="application/pdf" width="580" height="300">
                              <a href="/files/files/{{$file->file}}">{{$file->file}}</a>
                            </object>
                            @elseif($name[1]=='doc' || $name[1]=='docx')
                              <a href="/files/files/{{$file->file}}">{{$file->file}}</a>
                            @endif
                            <a href="{{route('admin.file.delete', ['publications', $item->id, $file->id])}}" style="" class="btn btn-danger waves-effect waves-light" name="button">Удалить</a>
                          </div>
                          <br>
                        @endforeach
                        <div class="form-group mb-0">
                            ...
                            <br>
                            <input type="file" name="pdf[1]" style="background: #999; border-radius: 4px; color: #fff; margin-bottom: 10px">
                        </div>
                        <div class="form-group" id="new_chq">
                        </div>
                        <input type="hidden" name="" value="1" id="total_chq">
                        <button type="button" class="add btn btn-info waves-effect waves-light" name="button">Добавить</button>
                        <button type="button" class="remove btn btn-danger waves-effect waves-light" name="button">Удалить</button>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-12">
                                <input class="form-control" name="video_uz"  type="text" value="{{$item->video_uz}}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="example-text-input" class="col-sm-4 col-form-label">Видео (ru)</label>
                            <div class="col-sm-12">
                                <input class="form-control" name="video_ru"  type="text" value="{{$item->video_ru}}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="example-text-input" class="col-sm-4 col-form-label">Видео (en)</label>
                            <div class="col-sm-12">
                                <input class="form-control" name="video_en"  type="text" value="{{$item->video_en}}">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="example-datetime-local-input" class="col-sm-4 col-form-label">Дата</label>
                            <div class="col-sm-12">
                                <input class="form-control" type="datetime-local" name="created_at" value="{{$item->created_at->format('Y-m-d\TH:i')}}" placeholder="2011-08-19T13:45:00" id="example-datetime-local-input">
                            </div>
                        </div>
...
<script>
    ...
    function add() {
      ...
      var new_input = '<div class="custom-file" id="new_'+new_chq_no+'"><input name="pdf['+new_chq_no+']" type="file" style="background: #999; border-radius: 4px; color: #fff; "  ></div>';
